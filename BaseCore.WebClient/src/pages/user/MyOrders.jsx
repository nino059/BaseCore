...
import { normalizeOrderList } from '../../utils/orderNormalize';
...
const MyOrders = () => {
  ...
  const loadOrders = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setOrders(normalizeOrderList(res));
    } catch (err) {
      console.error(err);
      setOrders([]);
      if (!silent) {
    ...
    <PublicLayout>
      <div className="bg-cream min-h-[80vh]">
        <div className="max-w-240 mx-auto px-5 py-12">
          ...
          {error && !loading && (
            <div className="mb-6 px-4 py-3 bg-[#fef2f2] border border-[#fecaca] text-[#991b1b] text-[0.88rem] flex items-center justify-between gap-3 flex-wrap">
              <span><i className="fas fa-exclamation-circle mr-2" />{error}</span>
              <button
                type="button"
                onClick={() => loadOrders()}
                className="px-4 py-2 bg-ink text-white border-none cursor-pointer text-[0.75rem] font-bold tracking-[0.08em] uppercase"
              >
                Thử lại
              </button>
            </div>
